...
export default {
  ...
  methods: {
    async TurnToInfo2 () {
      ...
      this.loading2 = true;
      const ress = await Promise.all(data.map(item => UserStore.Spot2Assets(item.currency, item.amount)));
      this.loading2 = false;
      for (const i in ress) {
        const res = ress[i];
    ...
    async FeatchCoins () {
      this.loading2 = true;
      const res = await UserStore.FeatchCoins();
      this.loading2 = false;
      if (res.Error()) this.$message.warning(res.Msg);
      const Coins = clone(UserStore.state.Coins);
      ...
      const data = Coins.map(data => {
        ...
        return Object.assign({}, data, {
          ...
          balanceUsdt, frozenUsdt, availableUsdt,
          type,
        });
      });
      data.sort((a, b) => {
